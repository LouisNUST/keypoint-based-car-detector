...
class Printer:
    ...
    def draw_boxes(self, axes, idx, color):
        ...
        yfinal = [self.boxes_gt[idx][i] for i in range(len(self.boxes_gt[idx])) if i%2==1]
        xfinal = [self.boxes_gt[idx][i] for i in range(len(self.boxes_gt[idx])) if i%2==0]
        
        ww_box_gt = max(xfinal)-min(xfinal)#self.boxes_gt[idx][2] - self.boxes_gt[idx][0]
        hh_box_gt = (max(yfinal)-min(yfinal))#(self.boxes_gt[idx][3] - self.boxes_gt[idx][1]) * self.y_scale
        ...
        #rectangle_gt = Rectangle((self.boxes_gt[idx][0], self.boxes_gt[idx][1] * self.y_scale), width=ww_box_gt, height=hh_box_gt, fill=False, color='g', linewidth=2)
        rectangle_gt = Rectangle((min(xfinal), min(yfinal) * self.y_scale), width=ww_box_gt, height=hh_box_gt, fill=False, color='g', linewidth=2)
