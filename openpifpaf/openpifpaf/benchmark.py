...
def cli():
    ...
    parser.add_argument('--long_edge', default=None, type=int,
                        help='long edges for the evaluation')
...
def run_eval_coco(output_folder, backbone, long_edge,eval_args):
    out_file = os.path.join(output_folder, backbone)+"_le_"+str(long_edge)+"_"
    ...
    if not (long_edge is  None ):
        subprocess.run([
            'python3', '-m', 'openpifpaf.eval_coco',
            '--output', out_file,
            '--checkpoint', backbone,
...
def main():
    ...
    for backbone in args.backbones:
        run_eval_coco(args.output, backbone, args.long_edge, eval_args)
