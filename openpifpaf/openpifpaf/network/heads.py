...
from ..data import COCO_KEYPOINTS, COCO_SKELETON
...
class CompositeField(Head, torch.nn.Module):
    ...
    def determine_nfields(head_name):
        ...
        print("COCO_SKELETON"+str(len(COCO_SKELETON)))
        print("COCO_KEYPOINTS"+str(len(COCO_KEYPOINTS)))
        if m is not None:
            return int(m.group(1))
        return {
            'paf': len(COCO_SKELETON),
            'pafb': len(COCO_SKELETON),
            'pafsb': len(COCO_SKELETON),
            'pafs19': len(COCO_SKELETON),
            'wpaf':len(COCO_SKELETON),
        }.get(head_name, len(COCO_KEYPOINTS))        #TODO
