...
def preprocess_monoloco(keypoints, kk):
    ...
    keypoints = torch tensors of (m, 3, nb_keypoints)  or list [3,nb_keypoints]
    Outputs =  torch tensors of (m, 3*nb_keypoints) in meters normalized (z=1) and zero-centered using the center of the box
    """
...
def preprocess_pifpaf(annotations, im_size=None):
    ...
    for dic in annotations:
        box = dic['bbox']
        if box[3] < 0.5or box[2]<0.5:  # Check for no detections (boxes 0,0,0,0)
            continue;
            #return [], []
        ...
        delta_h = box[3]*15/200#(box[3] - box[1]) / 7
        delta_w = box[2]*20/200#(box[2] - box[0]) / 3.5
        ...
        if(delta_h <=0 or delta_w<=0 ):
            ...
            if im_size is not None:
                box[0] = max(0, box[0])
                box[1] = max(0, box[1])
                box[2] = min(box[2] + box[0], im_size[0]) 
                box[3] = min(box[3] + box[1], im_size[1])
...
def prepare_pif_kps(kps_in):
    """Convert from a list of 51 to a list of 3, nb_keypoints"""

    assert len(kps_in) % 3 == 0, "keypoints expected as a multiple of 3"
    
    if True:
        #? Remove the keypoints related to the mirrors The final number of keypoints will be nb_keypoints-2
        #! If the if statement is set to True, you need to modify the value 'n_joints'(line 55) of train/trainer.py
        #! to (nb_keypoints-2) [in our case 12] and the value 'INPUT_SIZE'(line 20) of network/net.py to (nb_keypoints-2) *2 [In our case, 12*2]
        xxs = kps_in[0:][::3][:8] + kps_in[0:][::3][10:]
        yys = kps_in[1:][::3][:8] + kps_in[1:][::3][10:] # from offset 1 every 3
        ccs = kps_in[2:][::3][:8] + kps_in[2:][::3][10:]
    else:
        xxs = kps_in[0:][::3]
